    ...
    from (
        ...
        from ( 
            select 
                ...
                cat_name,
                node 
            from stg.zaful_eload_category
        ) a
    ) b
) m 
join stg.zaful_eload_goods n 
on m.cat_id=n.cat_id
where length(n.goods_sn)=9;
...
-- sku的价格、库存、是否24小时发货、sku的添加时间
    ...
    FROM(
	    ...
	    is_delete,
            goods_number,
	    case when (is_on_sale=1 and is_delete=0 and goods_number>0) then 1 else 0 end as sale_status,
            add_time
        FROM
...
GROUP BY
  a.goods_spu,
  a.dt) m
ON substr(n.goods_sn, 1, 7) = m.goods_spu
where length(n.goods_sn)=9;
...
select
    ...
    (case when b.node1 is not null then b.node1 else 'null' end) as item_cat1_id,
    (case when b.node2 is not null then b.node2 else 'null' end) as item_cat2_id,
    (case when b.node3 is not null then b.node3 else 'null' end) as item_cat3_id,
    (case when b.node4 is not null then b.node4 else 'null' end) as item_cat4_id,
    c.item_price,
    c.item_add_time,
    c.item_is_on_sale,
    c.item_is_delete,
    ...
    (case when c.item_star_level is not null then c.item_star_level else 0.0 end) as item_star_level,
    (case when c.item_review_num is not null then c.item_review_num else 0 end) as item_review_num
from (
    SELECT
        goods_sn,
        cat_id,
        ...
        node3,
        node4
    FROM zaful_recommend.sku_category
) b 
left join (
    SELECT
        item_id,
	...
	item_sale_status,
        ...
        item_star_level,
        item_add_time
    FROM zaful_recommend.zaful_item) c 
on b.goods_sn=c.item_id
