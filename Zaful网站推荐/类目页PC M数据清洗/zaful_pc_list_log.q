...
SET mapred.job.name='zaful_pc_list_log&order_info';
...
--最终结果数据汇总
CREATE TABLE IF NOT EXISTS dw_zaful_recommend.zaful_pc_list_log(
...
--最终结果数据汇总(数据拼接后)
CREATE TABLE IF NOT EXISTS dw_zaful_recommend.zaful_pc_list_log_new(
...
--埋点日志中间数据
CREATE TABLE IF NOT EXISTS dw_zaful_recommend.zaful_pc_list_log_tmp(
    glb_tm           string   COMMENT '时间戳', 
    page             string   COMMENT '页面索引', 
  	sku              string   COMMENT '商品SKU',
    price            string   COMMENT '当时的商品价格',
...
--中间结果写入最终分区结果表
INSERT OVERWRITE TABLE dw_zaful_recommend.zaful_pc_list_log  partition (`date` = '${ADD_TIME}')
SELECT * FROM dw_zaful_recommend.zaful_pc_list_log_tmp;


--最终结果拼接
INSERT OVERWRITE TABLE dw_zaful_recommend.zaful_pc_list_log_new  partition (`date` = '${ADD_TIME}')
SELECT
	x.platform,
	x.glb_t,
	x.glb_od,
...
FROM
	(
		SELECT
            ...
            glb_dc
		FROM
			dw_zaful_recommend.zaful_pc_list_log_tmp
	) x
GROUP BY
	x.platform,
	x.glb_t,
...
--订单表结果数据
CREATE TABLE IF NOT EXISTS dw_zaful_recommend.zaful_order_info(
...
--订单表中间数据
CREATE TABLE IF NOT EXISTS dw_zaful_recommend.zaful_order_info_tmp(
    user_id           string   COMMENT '用户ID', 
    sku               string   COMMENT 'sku', 
  	time              string   COMMENT '下单时间',
    platform          string   COMMENT '平台',
...
--订单表中间数据
INSERT OVERWRITE TABLE dw_zaful_recommend.zaful_order_info_tmp 
SELECT
  x.user_id,
  p.goods_sn AS sku,
  x.add_time AS time,
...
INSERT OVERWRITE TABLE dw_zaful_recommend.zaful_order_info  partition (`date` = '${ADD_TIME}')
SELECT
	x.user_id,
	x.sku,
	x.time,
...
FROM
	(
		...
		FROM
			dw_zaful_recommend.zaful_order_info_tmp m
