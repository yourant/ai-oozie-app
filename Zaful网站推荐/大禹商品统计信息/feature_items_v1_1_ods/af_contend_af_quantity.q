...
set hive.support.concurrency=false;                
 ...
 --输出结果表
  create table if not exists dw_proj.feature_items_v2_2_ods_order(
 sku                  STRING       comment '',
...
insert overwrite table dw_proj.feature_items_v2_2_ods_order    
select
sku,sum(goods_number),sum(m.pay_amount),platform
from (
SELECT
  p.goods_sn as sku,
