...
insert overwrite table temp_zaful_recommend.goods_order_day 
    ...
    from (
        ...
        join (select * from ods.ods_m_zaful_eload_order_goods where dt='${DATE}') g --dt = ${DATE}
        on 
    ...
    union all 
        ...
        join (select * from ods.ods_m_zaful_eload_order_goods where dt='${DATE}') n --dt = ${DATE}
        on 
...
set mapred.job.queue.name = root.ai.offline;
--按天写分区表：pc平台统计
insert overwrite table dw_zaful_recommend.feature_items_v2_2_ods partition (platform='pc', year=${YEAR},month=${MONTH},day=${DAY})  
select 
    item_id,
...
set mapred.job.queue.name = root.ai.offline;
--按天写分区表：m平台统计
insert overwrite table dw_zaful_recommend.feature_items_v2_2_ods partition (platform='m', year=${YEAR},month=${MONTH},day=${DAY})  
select 
    item_id,
...
set mapred.job.queue.name = root.ai.offline;
--按天写分区表：ios平台统计
insert overwrite table dw_zaful_recommend.feature_items_v2_2_ods partition (platform='ios',year=${YEAR},month=${MONTH},day=${DAY})  
select 
    item_id,
...
set mapred.job.queue.name = root.ai.offline;
--按天写分区表：android平台统计
insert overwrite table dw_zaful_recommend.feature_items_v2_2_ods partition (platform='android',year=${YEAR},month=${MONTH},day=${DAY})  
select 
    item_id,
...
CREATE TABLE IF NOT EXISTS dw_zaful_recommend.feature_items_v2_2_ods_info (
...
insert overwrite table dw_zaful_recommend.feature_items_v2_2_ods_info partition (year='${YEAR}',month='${MONTH}',day='${DAY}')  
select coalesce(a.item_id,b.sku) as item_id,
...
from 
    ...
    from 
    dw_zaful_recommend.feature_items_v2_2_ods
    where concat(year,month,day) = '${DATE}'

    ) a
    full join
    (
    select sku,create_order_sum,gmv_num,platform
    from 
    dw_proj.feature_items_v2_2_ods_order
    ) b
    on a.item_id=b.sku and a.platform=b.platform
;
...
--3天聚合
-------------------------------------------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS dw_zaful_recommend.feature_items_v2_2_ods_info_three_days (
    item_id String COMMENT '商品sku编号',
    pv_cnt bigint COMMENT '商品的pc端曝光数',
    pv_uv bigint COMMENT '商品的pc端曝光uv',
    pv_per_cnt double COMMENT '商品的pc端人均曝光数',    
...
insert overwrite table dw_zaful_recommend.feature_items_v2_2_ods_info_three_days partition (year='${YEAR}',month='${MONTH}',day='${DAY}')  
select 
item_id,
...
platform
from 
dw_zaful_recommend.feature_items_v2_2_ods_info
where 
...
--7天聚合
-------------------------------------------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS dw_zaful_recommend.feature_items_v2_2_ods_info_seven_days (
    item_id String COMMENT '商品sku编号',
    pv_cnt bigint COMMENT '商品的pc端曝光数',
    pv_uv bigint COMMENT '商品的pc端曝光uv',
    pv_per_cnt double COMMENT '商品的pc端人均曝光数',    
...
insert overwrite table dw_zaful_recommend.feature_items_v2_2_ods_info_seven_days partition (year='${YEAR}',month='${MONTH}',day='${DAY}')  
select 
item_id,
sum(pv_cnt) as pv_cnt,
null,--count(distinct pv_uv),
...
--15天聚合
-------------------------------------------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS dw_zaful_recommend.feature_items_v2_2_ods_info_fifteen_days (
    item_id String COMMENT '商品sku编号',
    pv_cnt bigint COMMENT '商品的pc端曝光数',
    pv_uv bigint COMMENT '商品的pc端曝光uv',
    pv_per_cnt double COMMENT '商品的pc端人均曝光数',    
...
insert overwrite table dw_zaful_recommend.feature_items_v2_2_ods_info_fifteen_days partition (year='${YEAR}',month='${MONTH}',day='${DAY}')  
select 
item_id,
sum(pv_cnt) as pv_cnt,
null,--count(distinct pv_uv),
...
--30天聚合
-------------------------------------------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS dw_zaful_recommend.feature_items_v2_2_ods_info_thirty_days (
    item_id String COMMENT '商品sku编号',
    pv_cnt bigint COMMENT '商品的pc端曝光数',
    pv_uv bigint COMMENT '商品的pc端曝光uv',
    pv_per_cnt double COMMENT '商品的pc端人均曝光数',    
...
insert overwrite table dw_zaful_recommend.feature_items_v2_2_ods_info_thirty_days partition (year='${YEAR}',month='${MONTH}',day='${DAY}')  
select 
item_id,
sum(pv_cnt) as pv_cnt,
null,--count(distinct pv_uv),
