--@author ZhanRui
--@date 2018年6月25日 
--@desc  算法所需数据导入mongo PC/M 加拿大每个SKU的统计信息

SET mapred.job.name=lr_base_info3;
...
FROM(
	...
	WHERE 
		glb_t = 'ic' AND concat(year,month,day) = '${ADD_TIME}'
         AND glb_skuinfo != '' AND glb_plf in ('pc','m') and geoip_country_name='Canada'
...
WHERE
  ...
  AND glb_t = 'ic'
  AND glb_x = 'ADT'
   AND glb_plf in ('pc','m') and geoip_country_name='Canada'
) tmp
group by
  date,
  glb_plf,
