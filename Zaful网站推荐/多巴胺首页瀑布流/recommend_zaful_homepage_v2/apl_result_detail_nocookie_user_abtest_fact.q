...
SET mapred.job.name='apl_result_detail_nocookie_user_abtest_fact';

USE dw_zaful_recommend;

CREATE TABLE IF NOT EXISTS result_detail_nocookie_user_abtest
(
...
CREATE TABLE IF NOT EXISTS apl_result_detail_nocookie_user_abtest_fact(
...
INSERT OVERWRITE TABLE apl_result_detail_nocookie_user_abtest_fact
SELECT
	region_id,
	platform,
	goods_sn,
...
FROM(
	...
	FROM(
		SELECT
			...
			score
		FROM
			result_detail_nocookie_user_abtest
		WHERE
...
SELECT
	...
	a.score
FROM
	apl_result_detail_nocookie_user_abtest_fact a
LEFT JOIN
	apl_sku_lang_fact  b
ON
	a.goodssn = b.goodssn
...
SELECT
	...
	NVL(score,0)  score
FROM
	apl_result_detail_nocookie_user_abtest_fact
ORDER BY 
	score;
