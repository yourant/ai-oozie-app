...
--输出结果表
  create table if not exists dw_proj.app_zaful_sku_cat_id_no_country_category_wuc_wishlist_temp(
 ...
 --执行查询，将结果写入hive表分区
insert overwrite table dw_proj.app_zaful_sku_cat_id_no_country_category_wuc_wishlist_temp PARTITION (add_time='${ADD_TIME}')
select
c.sku,
c.af_impression,
c.af_view_product,
...
(
			...
			(
			SELECT 
			a.sku as sku,a.platform as platform,
			...
			full join dw_proj.app_zaful_sku_cat_id_no_country_af_content_af_quantity_stg_create_wishlist_category p on j.sku=p.sku and j.platform=p.platform
			full join dw_proj.app_zaful_sku_cat_id_no_country_af_content_af_quantity_stg_purchase_wishlist_category q on j.sku=q.sku and j.platform=q.platform
) c
...
--输出结果表
  create table if not exists dw_proj.app_zaful_sku_cat_id_no_country_category_wuc_wishlist(
 sku                        STRING    comment '商品id',
 af_impression              INT       comment '曝光量',
 af_view_product            INT       comment '点击量',
 af_add_to_wishlist         	INT   COMMENT '收藏量',
 ...
 insert overwrite table dw_proj.app_zaful_sku_cat_id_no_country_category_wuc_wishlist PARTITION (add_time='${ADD_TIME}')
select 
sku,
sum(af_impression),
sum(af_view_product),
...
sum(sale_value_user),
sum(sale_value_num),
event_time,date,platform,cat_id
from 
dw_proj.app_zaful_sku_cat_id_no_country_category_wuc_wishlist_temp
where add_time='${ADD_TIME}'
group by sku,event_time,date,platform,cat_id
;
...
--总的
--输出结果表
  create table if not exists dw_proj.app_zaful_sku_cat_id_no_country_all_wuc_wishlist_temp(
 sku                        STRING    comment '商品id',
 af_impression              INT       comment '曝光量',
 af_view_product            INT       comment '点击量',
 af_add_to_wishlist         	INT   COMMENT '收藏量',
 ...
 --执行查询，将结果写入hive表分区
insert overwrite table dw_proj.app_zaful_sku_cat_id_no_country_all_wuc_wishlist_temp PARTITION (add_time='${ADD_TIME}')
select
c.sku,
c.af_impression,
c.af_view_product,
...
(
			...
			full join dw_proj.app_zaful_sku_cat_id_no_country_af_content_af_quantity_stg_create_wishlist p on j.sku=p.sku and j.platform=p.platform

			full join dw_proj.app_zaful_sku_cat_id_no_country_af_content_af_quantity_stg_purchase_wishlist q on j.sku=q.sku and j.platform=q.platform
) c 
left join (select goods_sn,cat_id from ods.ods_m_zaful_eload_goods where dt='${ADD_TIME}' group by goods_sn,cat_id) k on c.sku=k.goods_sn
; 
  
  
  create table if not exists dw_proj.app_zaful_sku_cat_id_no_country_all_wuc_wishlist(
 sku                        STRING    comment '商品id',
 af_impression              INT       comment '曝光量',
 af_view_product            INT       comment '点击量',
 af_add_to_wishlist         	INT   COMMENT '收藏量',
 ...
 --执行查询，将结果写入hive表分区
insert overwrite table dw_proj.app_zaful_sku_cat_id_no_country_all_wuc_wishlist PARTITION (add_time='${ADD_TIME}')
select 
sku,
sum(af_impression),
sum(af_view_product),
...
event_time,date,platform,cat_id
from 
dw_proj.app_zaful_sku_cat_id_no_country_all_wuc_wishlist_temp
where add_time='${ADD_TIME}'
group by sku,event_time,date,platform,cat_id
;
