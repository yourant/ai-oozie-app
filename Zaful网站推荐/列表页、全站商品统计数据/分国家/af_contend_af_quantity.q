...
CREATE TABLE IF NOT EXISTS dw_proj.app_u_map(
	dd					STRING		   COMMENT ''
)
...
insert OVERWRITE table  dw_proj.app_u_map PARTITION (date = '${ADD_TIME}')
select
'${ADD_TIME}'
;
                       
 --输出结果表
  create table if not exists dw_proj.app_zaful_sku_cat_id_no_country_af_content_af_quantity_stg_create_wishlist(
 sku                  STRING       comment '',
 ...
 ;                  
  ...
  --输出结果表
  create table if not exists dw_proj.app_zaful_sku_cat_id_no_country_af_content_af_quantity_stg_purchase_wishlist(
 sku                  STRING       comment '',
...
insert overwrite table dw_proj.app_zaful_sku_cat_id_no_country_af_content_af_quantity_stg_create_wishlist    
select
sku,count(order_sn),sum(goods_number),sum(m.pay_amount)/sum(goods_number) * count(order_sn),sum(m.pay_amount),platform
from (
SELECT
  p.goods_sn as sku,
...
insert overwrite table dw_proj.app_zaful_sku_cat_id_no_country_af_content_af_quantity_stg_purchase_wishlist    
select
sku,count(order_sn),sum(goods_number),sum(m.pay_amount)/sum(goods_number) * count(order_sn),sum(m.pay_amount),platform
from (
SELECT
...
CREATE TABLE IF NOT EXISTS dw_zaful_recommend.zaful_app_category_xxx_all_order_info_good_tmp_order_sn(
...
--订单表
INSERT OVERWRITE TABLE dw_zaful_recommend.zaful_app_category_xxx_all_order_info_good_tmp_order_sn
SELECT
  p.goods_sn,
  x.user_id,
  x.order_status,
...
--列表页订单表
create table if not exists dw_proj.zaful_app_fourteen_days_category_order_info_sku(
 sku                  STRING       comment '',
...
insert overwrite table dw_proj.zaful_app_fourteen_days_category_order_info_sku 
SELECT
  x2.goods_sn as sku,
  x2.order_sn,
  x2.goods_number,
...
from
  ...
  INNER JOIN dw_zaful_recommend.zaful_app_category_xxx_all_order_info_good_tmp_order_sn x2 ON x1.user_id = x2.user_id
  AND x1.goods_sn = x2.goods_sn
;


--  --列表页输出结果表
-- create table if not exists dw_proj.app_zaful_sku_cat_id_no_country_af_content_af_quantity_stg_create_wishlist_category(
--  sku                  STRING       comment '',
--  create_order_count               STRING       comment '',
--  create_order_sum                STRING       comment '',
--  gmv_user             STRING       comment '',
...
--   --列表页输出结果表
-- create table if not exists dw_proj.app_zaful_sku_cat_id_no_country_af_content_af_quantity_stg_purchase_wishlist_category(
--  sku                  STRING       comment '',
--  purchase_count             STRING       comment '',
--  purchase_sum               STRING       comment '',
--  sale_value_user              STRING       comment '',
...
--  ;     
 ...
 insert overwrite table dw_proj.app_zaful_sku_cat_id_no_country_af_content_af_quantity_stg_create_wishlist_category    
select
sku,count(order_sn),sum(goods_number),sum(m.pay_amount)/sum(goods_number) * count(order_sn),sum(m.pay_amount),platform
from (
SELECT
...
FROM
  (
    SELECT
      ...
      order_status
    FROM
      dw_proj.zaful_app_fourteen_days_category_order_info_sku
  ) x
) as m
...
insert overwrite table dw_proj.app_zaful_sku_cat_id_no_country_af_content_af_quantity_stg_purchase_wishlist_category    
select
sku,count(order_sn),sum(goods_number),sum(m.pay_amount)/sum(goods_number) * count(order_sn),sum(m.pay_amount),platform
from (
SELECT
