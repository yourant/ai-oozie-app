--@author LIUQINGFAN
--@date 2018年5月30日 
--@desc  商品商详页推荐位AB测试结果输出

SET mapred.job.name='apl_result_detailpage_zaful_abtest_w2v';
...
CREATE TABLE IF NOT EXISTS result_detail_page_abtest_w2v
(
...
CREATE TABLE IF NOT EXISTS apl_result_detailpage_zaful_abtest_w2v(
...
INSERT OVERWRITE TABLE  dw_zaful_recommend.apl_result_detailpage_zaful_abtest_w2v
SELECT
	NVL(a.goods_sn1,'')              ,
        NVL(a.platform,'')              ,
	NVL(b.goodssn,'')              ,
...
FROM
	(
		SELECT
			...
			score
		FROM
			dw_zaful_recommend.result_detail_page_abtest_w2v
		WHERE
