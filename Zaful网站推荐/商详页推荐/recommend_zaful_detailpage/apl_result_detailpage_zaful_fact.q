...
SET mapred.job.name='apl_result_homepage_zaful_fact';
...
CREATE TABLE IF NOT EXISTS apl_result_detailpage_zaful_fact(
...
INSERT OVERWRITE TABLE  dw_zaful_recommend.apl_result_detailpage_zaful_fact
SELECT
	NVL(a.goods_sn1,'')              ,
        NVL(a.platform,'')              ,
	NVL(b.goodssn,'')              ,
...
FROM
	(
		SELECT
			...
			score
		FROM
			dw_zaful_recommend.result_goodsdetail_lr
		WHERE
