...
SET mapred.job.name=apl_result_zf_system_email_zaful_cf;
...
INSERT OVERWRITE TABLE dw_zaful_recommend.apl_result_zf_system_email_zaful_cf SELECT
	...
	NVL (thumbextendurl, '') AS thumbextendurl,
		...
		FROM
			(
				SELECT
					...
					score
				FROM
					dw_zaful_recommend.result_zf_system_email_zaful_cf
				WHERE
					 concat_ws('-', year, month, day) = '${ADD_TIME}'
			) t1
		JOIN tmp.apl_zaful_result_attr_fact t3 ON t1.good_sn = t3.goodssn
	) tmp;