...
SET mapred.job.name=apl_gb_magpie_bridge_result_fact;
...
INSERT OVERWRITE TABLE dw_gearbest_recommend.apl_gb_magpie_bridge_result_fact SELECT
		...
		FROM
			(
				SELECT
					...
					score
				FROM
					dw_gearbest_recommend.gb_magpie_bridge_result_gtq
				WHERE
