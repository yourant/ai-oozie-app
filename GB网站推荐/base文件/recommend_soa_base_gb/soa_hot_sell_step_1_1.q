...
-- 最近15天订单数据统计
CREATE TABLE IF NOT EXISTS dw_gearbest_recommend.goods_spu_order_count_15days
(goods_spu STRING COMMENT '商品spu', category_id INT COMMENT '商品分类', qty INT COMMENT '订单商品数量',pipeline_code STRING COMMENT '渠道编码' ) 
ROW FORMAT DELIMITED FIELDS TERMINATED BY '\t' LINES TERMINATED BY '\n' STORED AS orc;
INSERT OVERWRITE TABLE dw_gearbest_recommend.goods_spu_order_count_15days
SELECT
  c.goods_spu,
  d.category_id,
  b.qty,
    ...
    FROM
      ...
      JOIN
        ...
        WHERE
          ...
          AND created_time > UNIX_TIMESTAMP(
            DATE_SUB(FROM_UNIXTIME(UNIX_TIMESTAMP(), 'yyyy-MM-dd'), 15),'yyyy-MM-dd'
          )
      )e
...
--最近15天热销前50数据
CREATE TABLE IF NOT EXISTS dw_gearbest_recommend.goods_spu_hotsell_15days
...
ROW FORMAT DELIMITED FIELDS TERMINATED BY '\t' LINES TERMINATED BY '\n' STORED AS orc;
INSERT OVERWRITE TABLE dw_gearbest_recommend.goods_spu_hotsell_15days
SELECT
  goods_spu,
  pipeline_code,
  level,
    ...
    FROM
      (
        ...
        SELECT
          ...
          SUM(c.qty) sellcount
        FROM
          dw_gearbest_recommend.goods_spu_order_count_15days c
          JOIN (
            SELECT
              level_2,id
            FROM
        ...
        SELECT
          ...
          SUM(e.qty) sellcount
        FROM
          dw_gearbest_recommend.goods_spu_order_count_15days e
          JOIN (
            SELECT
              level_3,id
            FROM
        ...
        SELECT
          ...
          SUM(g.qty) sellcount
        FROM
          dw_gearbest_recommend.goods_spu_order_count_15days g
          JOIN (
            SELECT
              level_4,id
            FROM
        ...
        SELECT
          ...
          SUM(i.qty) sellcount
        FROM
          dw_gearbest_recommend.goods_spu_order_count_15days i
          JOIN (
            SELECT
              level_5,id
            FROM
