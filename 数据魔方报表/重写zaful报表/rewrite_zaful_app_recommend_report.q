...
--输出结果表
CREATE TABLE IF NOT EXISTS dw_zaful_recommend.rewrite_zaful_app_recommend_report(
pv                        INT            COMMENT "页面PV",
uv                        INT            COMMENT "页面UV",
...
--输出结果表导出表
CREATE TABLE IF NOT EXISTS dw_zaful_recommend.rewrite_zaful_app_recommend_report_exp(
pv                        INT            COMMENT "页面PV",
uv                        INT            COMMENT "页面UV",
exp_num                   INT            COMMENT "商品曝光数",
sku_uv                    INT            COMMENT "查看商品UV",
...
--所有结果汇总
INSERT into TABLE dw_zaful_recommend.rewrite_zaful_app_recommend_report PARTITION (add_time = '${ADD_TIME}')
select
  NVL(a.pv,0),
  NVL(b.uv,0),
  c.exp_num,
  ...
  l.pay_uv,
  m.pay_amount,
  k.purchase_num / c.exp_num * 100,
  l.pay_uv / d.sku_uv * 100,
  n.collect_uv,
  o.collect_num,
  'android',
...
INSERT overwrite table dw_zaful_recommend.rewrite_zaful_app_recommend_report_exp
select
  *
FROM
  dw_zaful_recommend.rewrite_zaful_app_recommend_report
where
  add_time = '${ADD_TIME}'
;
