...
select 
	...
	a.glb_dc
from 
    (select sample_num,policy,glb_dc,add_time from tmp.rewrite_sample_num_tmp ) a 
    join
    (select exp_num,policy,glb_dc,add_time from tmp.rewrite_exp_num_tmp ) b 
	on a.add_time=b.add_time and a.glb_dc=b.glb_dc and a.policy=b.policy
	join
    (select click_num,policy,glb_dc,add_time from tmp.rewrite_click_num_tmp ) c 
	on a.add_time=c.add_time and a.glb_dc=c.glb_dc and a.policy=c.policy
	 join
    (select cart_num,policy,glb_dc,add_time from tmp.rewrite_cart_num_tmp ) d 
	on a.add_time=d.add_time and a.glb_dc=d.glb_dc and a.policy=d.policy
	 join
    (select purchase_num,policy,glb_dc,add_time from tmp.rewrite_purchase_num_tmp ) e 
	on a.add_time=e.add_time and a.glb_dc=e.glb_dc and a.policy=e.policy
	 join
    (select pay_amount,policy,glb_dc,add_time from tmp.rewrite_pay_amount_tmp ) f 
	on a.add_time=f.add_time and a.glb_dc=f.glb_dc and a.policy=f.policy
	 join
    (select pay_gmv,policy,glb_dc,add_time from tmp.rewrite_pay_gmv_tmp ) g 
	on a.add_time=g.add_time and a.glb_dc=g.glb_dc and a.policy=g.policy
